 ...
 * @expose
 */
module.exports = function(object, fields, order) {
  /**
   * @description
  ...
  if(_.isObject(fields) && !_.isEmpty(fields)) {
    return BASE_QUERY.replace(' *', fields.join());
  }
  ...
  var queryStr = [
    BASE_QUERY,
    _.isString(object) ? object  : query(object),  // concat base + queryString + order
    _.isDefined(order) ? _.isString(order) ? 'ORDER BY '+order : 'ORDER BY '+ _.keys(order) :'' //need to figure out how to define direction
  ].join(' ');
